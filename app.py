...
def login_admin():
    if request.method == "POST":
        ...
        if username == "deeksha" and password == "0111":
            session["role"] = "admin"
...
def student_dashboard():
    ...
    # Fetch all PDFs and filter for:
    # 1. Admin uploads for this class (uploaded_by not in doc)
    # 2. Student's own uploads
    ...
    for p in all_pdfs:
        ...
        if uploader == session["user_id"]:
            pdfs.append(p)
        elif not uploader and str(p.get("class")) == str(student_class):
            # Admin uploads (no uploader) for this class
            # Ensure class string comparison
            pdfs.append(p)
            
    return render_template("student_dashboard.html", quizzes=quizzes, pdfs=pdfs)

@app.route("/student-upload-pdf", methods=["POST"])
def student_upload_pdf():
    if session.get("role") != "student":
        return redirect(url_for("login"))
    ...
    file = request.files["file"]
    subject = request.form.get("subject") or "Personal Study"
    student_class = session.get("class")
    ...
    if file:
        ...
        # Generate a safer unique filename
        filename = f"student_{session['user_id']}_{int(datetime.now().timestamp())}_{original_filename}"
        filepath = os.path.join(app.config["UPLOAD_FOLDER"], filename)
        ...
        if not content or len(content.strip()) < 50:
            if os.path.exists(filepath):
                os.remove(filepath)
            flash("Could not extract enough text from this PDF. Please use a text-based PDF.", "danger")
            return redirect(url_for("student_dashboard"))
        ...
        flash("Personal PDF uploaded and processed successfully!", "success")
    return redirect(url_for("student_dashboard"))

@app.route("/delete-student-pdf/<pdf_id>")
def delete_student_pdf(pdf_id):
    if session.get("role") != "student":
        return redirect(url_for("login"))
    
    # Use string comparison for session ID and ensure it's their own PDF
    pdf = db.pdfs.find_one({"_id": str(pdf_id), "uploaded_by": session["user_id"]})
    ...
    if pdf:
        ...
        db.pdfs.delete_one({"_id": pdf["_id"]})
        flash("Personal study material deleted successfully.", "success")
    else:
